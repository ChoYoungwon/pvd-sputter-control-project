...
IO_Name_String_Map IO_STR_Map_Table[] = {
	...
	{ "CM$1_MID_CONTROL"				, _K_F_IO	,	CM_MID_CONTROL					,	0	} ,
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
int FOUP_SENSOR_CHECK_DELAY(int* nCommStatus)
{
	int i;
	int nResult;
	...
	for (i = 0; i < 10; i++) {
		...
		nResult = FOUP_SENSOR_CHECK(nCommStatus);

		if (!(*nCommStatus))
			return eNONE_0;

		if (nResult == ePRESENT2_2 ||
			nResult == eABSENT2_1) {
			return nResult;
		}
...
int FOUP_SENSOR_CHECK( int *nCommStatus ) { // return 0 = None, 1 = Absent, 2 = Present
	...
	int nData1, nData2;
	int nCommStatus1, nCommStatus2;

	nData1 = READ_DIGITAL( diLP_CM_Placed , &nCommStatus1 ); // 0 = Absent, 1 = Present
	nData2 = READ_DIGITAL( diLP_CM_Present , &nCommStatus2 ); // 0 = Absent, 1 = Present

	//printf("========== comm 1:[%d], 2:[%d], Data 1:[%d], 2:[%d]\n", nCommStatus1, nCommStatus2, nData1, nData2);

	if ( nCommStatus1 && nCommStatus2 ) { 
		*nCommStatus = 1;
		if (nData1 == ePRESENT_1 && nData2 == ePRESENT_1) return ePRESENT2_2;
		else if (nData1 == eABSENT_0  && nData2 == eABSENT_0) return eABSENT2_1;
		else return eNONE_0;
	}
	else { 
		*nCommStatus = 0; return eNONE_0; 
	}
}
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Module_Status FOUP_SENSOR_ERROR( BOOL bPresent , BOOL bHO_AVAL , int Line ) {

	int nAlarm, nAlmResult;
	int nCommStatus; 
	int nCheck_Value;
	BOOL bCheck = FALSE;

	if (bPresent) { 
		nCheck_Value = eABSENT2_1; 
		nAlarm = ALARM_FOUP_PRESENT; 
	}
	else { 
		nCheck_Value = ePRESENT2_2;		
	...
	while ( TRUE ) {
		...
		//nSensorStatus = FOUP_SENSOR_CHECK_DELAY(&nCommStatus);

		//if (nSensorStatus != nCheck_Value) {
		//	_iLOG_PRINTF("  =>> FOUP_SENSOR_ERROR Trg[%d] Chk[%d] Line[%d]\n", nCheck_Value, nSensorStatus, Line);
		if ( FOUP_SENSOR_CHECK( &nCommStatus ) != nCheck_Value ) {
			
			_iLOG_PRINTF( "  =>> FOUP_SENSOR_ERROR Trg[%d] Chk[%d] Line[%d]\n" , nCheck_Value, FOUP_SENSOR_CHECK( &nCommStatus ), Line );
			...
			while( TRUE ) {
				...
				if ( ALARM_STATUS( nAlarm ) != ALM_PAUSED ) {
					nAlmResult = ALARM_STATUS( nAlarm );
					break;
				}
			} // recovery is only ignore and abort

			if ( nAlmResult == ALM_IGNORE ) return SYS_ERROR;
...
Module_Status AGV_LOAD_CONTROL() {
	...
	while( TRUE ) {
		...
		nStatus = FOUP_SENSOR_ERROR( TRUE , TRUE , __LINE__ );
	...
	while( TRUE ) {
		...
		nStatus = FOUP_SENSOR_ERROR( FALSE , TRUE , __LINE__ );
...
Module_Status Load_Control() {
	...
	while ( TRUE ) { // 1st while() : Carrier Load
		...
		if ( gbControl_AutoMode ) { // Auto Mode
			...
			nStatus = FOUP_SENSOR_ERROR( TRUE , FALSE , __LINE__ ); // Foup check before load
...
Module_Status Unload_Control() {
	...
	while ( TRUE ) { // 1st while() : Wait Unload SW or AGV
		...
		if ( gbControl_AutoMode ) { // Auto Mode
			...
			nStatus = FOUP_SENSOR_ERROR( FALSE , FALSE , __LINE__ ); // Foup check before unload
