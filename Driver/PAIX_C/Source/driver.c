...
int Digital_In(int ID1, int ID2, int ID3, int ID4, int* Result) {
	...
	{
		short a, b, c, d;
		nRet = nmc_GetDeviceInfo(m_nDev_no, &a, &b, &c, &d);
		//printf("a=%d, b=%d, c=%d, d=%d\n", a, b, c, d);
		switch (ID2)
		{
		case 1: // diDevice_Count
			if (a == 0)ReadData = 0;
			else if (a == 2)ReadData = 1;
			else if (a == 4)ReadData = 2;
			else if (a == 6)ReadData = 3;
			else if (a == 8)ReadData = 4;
			else ReadData = -1;
			break;
		case 2: // diDevice_TypeDIO
			if (b == 0)ReadData = 0;
			else if (b == 1)ReadData = 1;
			else if (b == 2)ReadData = 2;
			else if (b == 3)ReadData = 3;
			else if (b == 4)ReadData = 4;
			else ReadData = -1;
			break;
		case 3:
			if (c == 0)ReadData = 0;
			else if (c == 1)ReadData = 1;
			else if (c == 2)ReadData = 2;
			else ReadData = -1;
			break;
		case 4:
			if (d == 0)ReadData = 0;
			else if (d == 1)ReadData = 1;
			else ReadData = -1;
			break;
		default:
			ReadData = -1;
